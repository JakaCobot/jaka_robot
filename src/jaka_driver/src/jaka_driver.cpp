                         ...
                         jaka_msgs::Move::Response &response)
{
    ...
    /*
    norm()是 Frobenius 范数, 所有元素平方之和的平方根
    normalize() 会将向量本身改变进行归一化
    normalized() 会返回一个归一化向量，但不会改变向量本身
    */
        ...
        jaka_msgs::Move::Response &response)
{
    ...
    switch (request.coord_mode)
    {
        ...
        case 2:
            ...
            break; 
        default:
            ROS_INFO("坐标系输入错误，请重新输入");
            return true;
    }
    // 4 确定速度方向 
    //判断机器人运动（关节或者笛卡尔）正方向还是负方向
    if(request.index&1)
    {
                     ...
                     jaka_msgs::ServoMove::Response &response)
{
    ...
    joint_pose.jVal[4] = request.pose[4];
    joint_pose.jVal[5] = request.pose[5];
    int ret = robot.servo_j(&joint_pose, MoveMode::ABS);
    switch(ret)
    {
        case 0:
            response.ret = 1;
