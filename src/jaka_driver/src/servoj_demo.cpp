...
//         cout << "调用servo_j的返回值:" << servo_pose.response.ret << "  ";
...
int main(int argc, char *argv[])
{
    ...
    //ROS_INFO("hhashlhdlsahdksdhlakdhksdhlkadhksdlka");
    ros::Duration(3).sleep();

// /home/jaka/Desktop/jaka_robot/src/jaka_driver/src/jjj.csv

	ifstream infile("/home/jaka/Desktop/jaka_robot/src/jaka_driver/src/jjj.csv", ios::in);
	string line;
...
//for (int i = 0; i < 400; i++) 
{
    cout<<posVector[i].pos1<<endl;
    double pose[] ={posVector[i].pos1,posVector[i].pos2,posVector[i].pos3,posVector[i].pos4,posVector[i].pos5,posVector[i].pos6 };
                
        for (int j =0; j < 6; j++)
        {
            servo_pose.request.pose.push_back(pose[j]);
...
//     ifstream infile("/home/jaka/Desktop/jaka_robot/src/jaka_driver/src/sdp.csv", ios::in);
// 	string line;
// 	vector<struct  Position> posVector;
// 	getline(infile, line);
// 	while (getline(infile, line)) {
